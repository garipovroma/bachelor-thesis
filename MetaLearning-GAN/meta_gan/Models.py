...
class GeneratorWithoutMeta(nn.Module):

    def __init__(self, features_size: int, instances_size: int, classes_size: int, meta_length: int, z_length: int):
        super(GeneratorWithoutMeta, self).__init__()
...
class DiscriminatorWithoutMeta(nn.Module):
    def __init__(self, features_size: int, instances_size: int, classes_size: int, meta_length: int,
                 lambda_length: int):
        super(DiscriminatorWithoutMeta, self).__init__()
        ...
        self.fc = nn.Linear(in_features=self.data_size * 16, out_features=self.lambda_length + 1)
...
def get_discriminator_without_meta():
    features = CONFIG.features
    instances = CONFIG.instances
    classes = CONFIG.classes
    metas_length = CONFIG.meta_size
    lambdas_length = CONFIG.out_classes
    discriminator = DiscriminatorWithoutMeta(features, instances, classes, metas_length, lambdas_length)
    return discriminator